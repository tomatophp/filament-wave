<div x-data="{ open: false }" class="flex h-full md:flex-1">
    ...
    <div class="flex sm:ml-6 sm:items-center">
        ...
        @if( auth('accounts')->user()->onTrial() )
            <div class="hidden relative justify-center items-center h-full md:flex">
                <span class="px-3 py-1 text-xs text-red-600 bg-red-100 rounded-md border border-zinc-200">You have {{ auth('accounts')->user()->daysLeftOnTrial() }} @if(auth('accounts')->user()->daysLeftOnTrial() > 1){{ 'Days' }}@else{{ 'Day' }}@endif left on your Trial</span>
            </div>
        @endif
        ...
        <div @click.away="open = false" class="flex relative items-center ml-3 h-full" x-data="{ open: false }">
            <div>
                <button @click="open = !open" class="flex text-sm rounded-full border-2 border-transparent transition duration-150 ease-in-out focus:outline-none focus:border-zinc-300" id="user-menu" aria-label="User menu" aria-haspopup="true" x-bind:aria-expanded="open" aria-expanded="true">
                    <img class="w-8 h-8 rounded-full" src="{{ auth('accounts')->user()->avatar() . '?' . time() }}" alt="{{ auth('accounts')->user()->name }}'s Avatar">
                </button>
            </div>
            ...
            <div
                ...
                <div class="bg-white rounded-xl border shadow-md border-zinc-100" role="menu" aria-orientation="vertical" aria-labelledby="options-menu">
                    <a href="{{ route('wave.profile', auth('accounts')->user()->username) }}" class="block px-4 py-3 text-zinc-700 hover:text-zinc-800">

                        <span class="block text-sm font-medium leading-tight truncate">
                            {{ auth('accounts')->user()->name }}
                        </span>
                    ...
                    <div class="py-1">
                        ...
                        <div class="block px-4 py-1">
                            <span class="inline-block px-2 my-1 -ml-1 text-xs font-medium leading-5 rounded text-zinc-600 bg-zinc-200">{{ auth('accounts')->user()->roles->first()->name }}</span>
                        </div>
                        @trial
                        ...
                        @endtrial
                        @if( !auth('accounts')->guest() && auth('accounts')->user()->isAdmin() )
                        ...
                        @endif
                        <a href="{{ route('wave.profile', auth('accounts')->user()->username) }}" class="block px-4 py-2 text-sm leading-5 text-zinc-700 hover:bg-zinc-100 hover:text-zinc-900 focus:outline-none focus:bg-zinc-100 focus:text-zinc-900">My Profile</a>
