...
<x-dynamic-component :component="((auth('accounts')->guest()) ? 'layouts.marketing' : 'layouts.app')" bodyClass="bg-zinc-50">
    @volt('wave.profile')

        <x-dynamic-component :component="((auth('accounts')->guest()) ? 'container' : 'app.container')">
            ...
            <div class="flex lg:flex-row flex-col @guest pb-20 pt-10 @endguest space-x-5 h-full">
                <x-card class="flex flex-col justify-center items-center p-10 w-full lg:w-1/3">
                        ...
                        @if (auth('accounts')->check() && auth('accounts')->user()->isAdmin())
