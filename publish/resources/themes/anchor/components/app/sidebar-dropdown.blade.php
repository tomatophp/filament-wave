<div x-data="{ {{ $id }}: {{ $open ?? false }} }"
    ...
    >
        <div class="flex relative items-center w-full h-auto">
            <x-dynamic-component :component="$icon" class="flex-shrink-0 ltr:mr-2 rtl:ml-2 w-5 h-5" />
            <span class="ltr:mr-0 rtl:ml-0">{{ $text }}</span>
            <span :class="{ 'rotate-180' : {{ $id }} == true }" class="ltr:mr-0.5 rtl:ml-0.5 ltr:ml-auto rtl:mr-auto w-4 h-4 duration-300 ease-out">
                <x-phosphor-caret-down class="w-full h-full" />
            </span>
        </div>

        <template x-teleport="#{{ $id }}">
            <div class="relative p-1 gap-1" x-show="{{ $id }}" x-collapse x-cloak>
                {{ $slot }}
            </div>
        </template>
    </div>
