...
>
    @if($dismissable)
        <button @click="alert_{{ $id }}=false" class="absolute ltr:right-0 rtl:left-0 top-0 z-50 p-1.5 ltr:mr-3 rtl:ml-3 rounded-full opacity-70 mt-3.5 cursor-pointer hover:opacity-100 hover:bg-zinc-200 hover:dark:bg-zinc-700 hover:dark:text-zinc-300 text-zinc-500 dark:text-zinc-400"><x-phosphor-x-bold class="w-3.5 h-3.5" /></button>
    @endif
    @if($title ?? false)
    ...
    @endif
    <div class="@if($title ?? false){{ 'ltr:pl-7 rtl:pr-7' }}@endif text-sm leading-6">{{ $slot }}</div>
